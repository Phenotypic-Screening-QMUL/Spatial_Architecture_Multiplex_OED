#Run script first, then 12 (PCA) then 13  (Stat test)
...
Intensity_Data <- Slide_Summary[, c(5:13)] #%>% scale() %>% as.data.frame() - scaling not required as RF , scaling for PCA performed later
...
CN_Data      <- fread("Neighbourhood_Region_Data.csv", encoding = "Latin-1") %>% select(-V1)
...
subset_list <- list(
  ...
  CN       = All_Means %>% select(Slide, Transformation, Clinical_Class, all_of(CN_vars)),
...
k <- 10
all_results_rf <- list()
roc_objects    <- list()  # store roc objects to compute bootstrap CIs AFTER the main loop
...
for (subset_name in names(subset_list)) {
  ...
  roc_objects[[subset_name]] <- roc_obj  # saved for post-loop bootstrap CI
...
# 6b. Bootstrap 95% CI for AUC (run AFTER the main loop, so it cannot shift
# the RNG stream for any subset's folds/RF models above - Accuracy, Kappa,
# Sensitivity, Specificity, Precision, Recall, F1, AUC, and confusion
# matrices are all identical to the base script with no CI addition.)

auc_ci_list <- list()

for (subset_name in names(roc_objects)) {
  auc_ci <- ci.auc(
    roc_objects[[subset_name]],
    conf.level = 0.95,
    method = "bootstrap",
    boot.n = 2000,
    stratified = TRUE,
  ...
  auc_ci_list[[subset_name]] <- tibble(
    Subset = subset_name,
    AUC_CI = paste0("+/-", round((auc_ci[3] - auc_ci[1]) / 2, 3))
  )
}

auc_ci_df <- bind_rows(auc_ci_list)
...
flat_metrics <- bind_rows(lapply(all_results_rf, function(x) x$Metrics)) %>%
  left_join(auc_ci_df, by = "Subset") %>%
  relocate(AUC_CI, .after = AUC)
...
flat_probs <- bind_rows(lapply(all_results_rf, function(x) x$CV_Probabilities), .id = "Subset")
